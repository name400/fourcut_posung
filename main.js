...
/* ===== storage: idb-keyval → localStorage fallback ===== */
const idb = (window.idbKeyval && typeof window.idbKeyval.set==='function') ? {
  set: window.idbKeyval.set, get: window.idbKeyval.get, keys: window.idbKeyval.keys, del: window.idbKeyval.del
} : {
...
function ensureHTTPS(){
  if (!location.protocol.startsWith('https') && location.hostname!=='localhost'){
    console.warn('Camera requires HTTPS on mobile.');
  }
}
...
async function startCamera(){
  ...
  try{
    ...
    if (e.name==='NotAllowedError' || e.name==='SecurityError') {
      msg = '브라우저/OS 권한에서 카메라 허용이 필요합니다.';
    } else if (!window.isSecureContext) {
      msg = 'HTTPS(또는 localhost)에서만 카메라 사용 가능.';
...
btnGallery?.addEventListener('click', async ()=>{ await renderGallery(); openGallery(); });
