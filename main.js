...
async function startCamera(){
  try{
    ...
    if(e.name==='NotAllowedError' || e.name==='SecurityError'){
      msg = '브라우저 권한에서 카메라 허용 필요(인앱 브라우저 대신 Chrome/Safari 권장).';
    }else if(!window.isSecureContext){
      msg = 'HTTPS(또는 localhost)에서만 카메라 사용 가능.';
    }else if(e.name==='NotFoundError'){
      msg = '카메라 장치를 찾지 못했습니다.';
    }else{
      msg += `: ${e.message || e}`;
    }
    alert(msg);
...
btnReset.onclick=()=>{
  ...
  renderThumbs(); renderPreview(); updateCounter(); if(busyEl) busyEl.hidden=true;
};
btnShot.onclick=()=>{
